...
import whu.se.interpret.po.ProjectSet;
    ...
    **/
    public ArrayList<Node> getProductionSet(ArrayList<Node> core){
        ...
        for (Node node:core
            ...
            //获取核心项目在.后面的第一个符号，例如A->.By,afterPoint就是B
            String afterPoint = node.getRight().get(node.getIndex());
            //如果是非终结符号,就将所有以afterPoint开头的产生式加入到非核心项目集
            if(!isTerm(afterPoint)){
                for (Node n:grammar
                     ) {
                    if (n.getLeft().equals(afterPoint))
                        array.add(n);
                }
            }
        }
